...
def render_sync_status_widget():
    """Render a compact sync status widget in the sidebar."""
    with st.sidebar:
        st.subheader("🔄 Sync Status")

        if not job_scheduler.scheduler.running:
            st.warning("⏸️ Scheduler not running")
            if st.button("▶️ Start Scheduler", key="start_scheduler_sidebar"):
                job_scheduler.start()
                log_info("Background scheduler started from sidebar", 
        ...
        recent_jobs = []  # TODO: Get from job_scheduler

        if recent_jobs:
            st.write("**Recent Syncs:**")
            for job in recent_jobs[-3:]:  # Show last 3
                status_icon = "✅" if job.get('status') == 'success' else "❌"
                st.write(f"{status_icon} {job.get('name', 'Unknown')} - {job.get('time', 'Unknown')}")
        else:
