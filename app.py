...
def load_preferences(df: pd.DataFrame | None = None, state=None) -> None:
    ...
    set_default("account_size", 10000.0)
    set_default("risk_per_trade", 0.01)
    set_default("max_daily_loss", 500.0)

    if df is not None:
        set_default("symbols", df["symbol"].unique().tolist())
        set_default("directions", df["direction"].unique().tolist())
        set_default("brokers", df["broker"].unique().tolist())
        set_default(
            "date_range",
            [df["entry_time"].min().date(), df["exit_time"].max().date()],
        )
...
def main() -> None:
    ...
    with st.sidebar:
        theme = st.selectbox("Theme", ["Light", "Dark"], index=0, key="theme")
        if theme == "Dark":
            st.markdown(
        ...
        st.caption("We do not store or share your uploaded trade data.")
    ...
    if selected_file:
        ...
        if importer.validate_columns(df):
            ...
            for col in REQUIRED_COLUMNS:
                mapping[col] = st.selectbox(
                    f"Column for {col}", options=df.columns, key=col
                )
            try:
        ...
        with st.sidebar.expander("Filters", expanded=True):
            ...
            directions = st.multiselect(
                ...
                default=st.session_state.directions,
                key="directions",
            )
            brokers = st.multiselect(
                "Broker",
                options=df["broker"].unique().tolist(),
                default=st.session_state.brokers,
                key="brokers",
            )
        ...
        risk_box = st.container()
        with risk_box:
            st.subheader("Risk Assessment")
            col1, col2, col3 = st.columns(3)
            account_size = col1.number_input(
                "Account Size",
                value=st.session_state.account_size,
                key="account_size",
            )
            risk_per_trade = col2.number_input(
                "Risk % per Trade",
                value=st.session_state.risk_per_trade,
                key="risk_per_trade",
            )
            max_daily_loss = col3.number_input(
                "Max Daily Loss",
            ...
            if st.button("Assess Risk"):
                risk = assess_risk(
                    filtered_df, account_size, risk_per_trade, max_daily_loss
                )
                if risk["risk_alert"]:
