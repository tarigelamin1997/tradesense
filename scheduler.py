...
try:
    from logging_manager import log_error, log_info, log_warning, log_sync_result, LogCategory
...
class JobScheduler:
    ...
    def _setup_event_listeners(self):
        ...
        def job_listener(event):
            if event.exception:
                ...
                logger.error(f"Job {event.job_id} failed: {error_msg}")
                if CENTRALIZED_LOGGING:
                    log_error(category=LogCategory.SCHEDULER, message=f"Job {event.job_id} failed: {error_msg}", job_id=event.job_id)
                self._update_job_status(event.job_id, JobStatus.FAILED, str(event.exception))
            else:
                logger.info(f"Job {event.job_id} completed successfully")
                if CENTRALIZED_LOGGING:
                    log_info(category=LogCategory.SCHEDULER, message=f"Job {event.job_id} completed successfully", job_id=event.job_id)
    ...
    def start(self):
        ...
        if not self.scheduler.running:
            ...
            if CENTRALIZED_LOGGING:
                log_info(category=LogCategory.SCHEDULER, message="Job scheduler started")
    ...
    def stop(self):
        ...
        if self.scheduler.running:
            ...
            if CENTRALIZED_LOGGING:
                log_info(category=LogCategory.SCHEDULER, message="Job scheduler stopped")
    ...
    def schedule_connector_sync(self, 
        ...
        if CENTRALIZED_LOGGING:
            log_info(category=LogCategory.SCHEDULER, message=f"Scheduled connector sync job: {job_id}", job_id=job_id)
        return job_id
    ...
    def execute_manual_sync(self, 
        ...
        if CENTRALIZED_LOGGING:
            log_info(category=LogCategory.SCHEDULER, message=f"Executing manual sync job: {job_id}", job_id=job_id)
        return job_id
    ...
    def _execute_connector_sync(self, job_id: str, config: Dict[str, Any]):
        ...
        try:
            ...
            if CENTRALIZED_LOGGING:
                log_info(category=LogCategory.SYNC, message=f"Starting connector sync: {connector_name} for user {user_id}", job_id=job_id, user_id=user_id, connector_name=connector_name)
            ...
            if raw_trades:
                ...
                if CENTRALIZED_LOGGING:
                    log_sync_result(user_id=user_id, connector_name=connector_name, job_id=job_id, records_processed=records_processed, execution_time=execution_time)
            else:
                ...
                if CENTRALIZED_LOGGING:
                    log_info(category=LogCategory.SYNC, message=f"No new data found for {connector_name}", job_id=job_id, user_id=user_id, connector_name=connector_name)

        except Exception as e:
            error_msg = str(e)
            logger.error(f"Sync job {job_id} failed: {error_msg}")
            if CENTRALIZED_LOGGING:
                log_error(category=LogCategory.SYNC, message=f"Sync job {job_id} failed: {error_msg}", job_id=job_id, user_id=user_id, connector_name=connector_name)
    ...
    def _get_user_credentials(self, user_id: int, connector_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if CENTRALIZED_LOGGING:
                log_error(category=LogCategory.CREDENTIALS, message=f"Error retrieving credentials: {e}", user_id=user_id, connector_name=connector_name)
            return None
    ...
    def _save_synced_data(self, user_id: int, df: pd.DataFrame, source: str) -> int:
        ...
        try:
            ...
            if CENTRALIZED_LOGGING:
                log_error(category=LogCategory.DATA_STORAGE, message=f"Error saving synced data: {e}", user_id=user_id, connector_name=source)
            raise
    ...
    def _handle_job_error(self, job_id: str, error_message: str):
        ...
        if result:
            ...
            if error_count >= 5:
                ...
                if CENTRALIZED_LOGGING:
                    log_warning(category=LogCategory.SCHEDULER, message=f"Job {job_id} disabled after 5 failures", job_id=job_id)
    ...
    def cancel_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            if CENTRALIZED_LOGGING:
                log_info(category=LogCategory.SCHEDULER, message=f"Job {job_id} cancelled successfully", job_id=job_id)
            return True

        except Exception as e:
            logger.error(f"Error cancelling job {job_id}: {e}")
            if CENTRALIZED_LOGGING:
                log_error(category=LogCategory.SCHEDULER, message=f"Error cancelling job {job_id}: {e}", job_id=job_id)
            return False
    ...
    def _cleanup_old_jobs(self):
        ...
        if CENTRALIZED_LOGGING:
            log_info(category=LogCategory.SCHEDULER, message=f"Cleaned up {deleted_count} old job history records", records_deleted=deleted_count)
    ...
    def _check_credential_expiry(self):
        ...
        if CENTRALIZED_LOGGING:
            log_info(category=LogCategory.CREDENTIALS, message="Checking credential expiry...")
