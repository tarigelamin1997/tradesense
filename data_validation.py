...
class DataValidator:
    ...
    def validate_and_clean_data(self, df: pd.DataFrame, interactive: bool = True) -> Tuple[pd.DataFrame, Dict]:
        """
        Validate and clean trade data with interactive correction options.
        
        Returns:
            tuple: (cleaned_dataframe, validation_report)
        """
        ...
        # Step 1: Basic data type validation and correction
        cleaned_df, type_corrections = self._fix_data_types(cleaned_df)
        report['corrections_made'].extend(type_corrections)
        
        # Step 2: Column-by-column validation
        for column in cleaned_df.columns:
            if column in self.validation_rules:
                cleaned_df, column_issues = self._validate_column(cleaned_df, column, interactive)
                report['issues_found'].extend(column_issues)
        
        # Step 3: Cross-column logical validation
        cleaned_df, logic_issues = self._validate_logical_consistency(cleaned_df, interactive)
        report['issues_found'].extend(logic_issues)
        
        # Step 4: Remove completely invalid rows
        cleaned_df = self._remove_invalid_rows(cleaned_df)
        
        # Step 5: Calculate final metrics
        report['final_rows'] = len(cleaned_df)
        report['data_quality_score'] = self._calculate_quality_score(original_count, len(cleaned_df), len(report['issues_found']))
