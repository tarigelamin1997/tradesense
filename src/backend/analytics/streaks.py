...
def calculate_win_loss_streaks(trades: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def get_entry_time(x):
        return x.get('entry_time', '') if isinstance(x, dict) else getattr(x, 'entry_time', '')
    sorted_trades = sorted(trades, key=get_entry_time)
    ...
    for trade in sorted_trades:
        ...
        if pnl is None:
            continue  # skip trades with None PnL
    ...
    total_streaks = total_win_streaks + total_loss_streaks
...
def calculate_max_consecutive_wins(trades: List[Dict[str, Any]]):
    """
    Return dict with details of max consecutive wins: length, pnl, trades.
    """
    return _find_max_streak_details(trades, is_win=True)


def calculate_max_consecutive_losses(trades: List[Dict[str, Any]]):
    """
    Return dict with details of max consecutive losses: length, pnl, trades.
    """
    return _find_max_streak_details(trades, is_win=False)
...
# Helper for max streak details
def _find_max_streak_details(trades, is_win=True):
    def get_pnl(t):
        return t.get('pnl', 0) if isinstance(t, dict) else getattr(t, 'pnl', 0)
    def get_entry_time(t):
        return t.get('entry_time', '') if isinstance(t, dict) else getattr(t, 'entry_time', '')
    sorted_trades = sorted(trades, key=get_entry_time)
    max_streak = 0
    max_streak_trades = []
    current_streak = 0
    ...
    for trade in sorted_trades:
        ...
        if pnl is None:
            if current_streak > max_streak:
                ...
                max_streak_trades = current_trades[:]
            current_streak = 0
            current_trades = []
            continue
    ...
    if current_streak > max_streak:
        max_streak = current_streak
        max_streak_trades = current_trades[:]
    streak_pnl = sum(get_pnl(t) for t in max_streak_trades if get_pnl(t) is not None)
    return {
        'length': max_streak,
        'pnl': streak_pnl,
