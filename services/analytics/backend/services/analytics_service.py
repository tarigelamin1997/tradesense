...
class AnalyticsService:
    ...
    def _calculate_overview(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Calculate basic analytics overview for test compatibility"""
        total_trades = len(df)
        winning_trades = len(df[df['pnl'] > 0])
        losing_trades = len(df[df['pnl'] < 0])
    ...
    def _calculate_performance_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        """Calculate performance metrics for test compatibility"""
        sharpe_ratio = self._calculate_sharpe_ratio(df['pnl'])
        # For sortino_ratio, use downside std
        downside = df[df['pnl'] < 0]['pnl']
        if len(downside) > 0:
            sortino_ratio = df['pnl'].mean() / downside.std() if downside.std() != 0 else 0
        else:
            sortino_ratio = 0
        average_win = df[df['pnl'] > 0]['pnl'].mean() if len(df[df['pnl'] > 0]) > 0 else 0
        average_loss = df[df['pnl'] < 0]['pnl'].mean() if len(df[df['pnl'] < 0]) > 0 else 0
        return {
